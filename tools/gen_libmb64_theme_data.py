...
def parse_material_defs(data_text: str, surfaces: dict[str, int]) -> tuple[list[tuple[str, int]], list[tuple[str, str]]]:
    ...
    entry_re = re.compile(
        r"{[^,]+,\s*(MAT_[A-Z0-9_]+)\s*,\s*[^,]+,\s*(SURFACE_[A-Z0-9_]+)\s*,.*//\s*(MB64_MAT_[A-Z0-9_]+)"
    )
    for raw in body.splitlines():
        match = entry_re.search(raw)
        if not match:
            continue
        material_type, surface_name, mat_name = match.groups()
    ...
    return surface_entries, type_entries
...
def main() -> int:
    ...
    material_surfaces, material_types = parse_material_defs(data_text, surfaces)
