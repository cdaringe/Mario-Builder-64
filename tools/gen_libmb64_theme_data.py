...
def parse_material_surfaces(data_text: str, surfaces: dict[str, int]) -> list[tuple[str, int]]:
    body = extract_initializer(data_text, "struct mb64_material mb64_mat_table[]")
    entries: list[tuple[str, int]] = []
    entry_re = re.compile(r"{[^,]+,\s*[^,]+,\s*[^,]+,\s*(SURFACE_[A-Z0-9_]+)\s*,.*//\s*(MB64_MAT_[A-Z0-9_]+)")
    for raw in body.splitlines():
        match = entry_re.search(raw)
        if not match:
            continue
        surface_name, mat_name = match.groups()
        ...
        entries.append((mat_name, surfaces[surface_name]))
    if not entries:
        raise ValueError("no material surfaces parsed")
    return entries
...
def main() -> int:
    ...
    material_surfaces = parse_material_surfaces(data_text, surfaces)
