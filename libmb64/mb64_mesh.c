...
static int faceshape_has_full_bottom(uint8_t faceshape) {
    return faceshape == MB64_FACESHAPE_FULL ||
           (faceshape >= MB64_FACESHAPE_UPPERGENTLE_1 &&
            faceshape <= MB64_FACESHAPE_BOTTOMSLAB);
}

static int should_cull_topslab_check(const mb64_level_t *level,
                                     const mb64_tile_t *t,
                                     uint8_t direction) {
    int dx = 0;
    int dy = 0;
    ...
    const uint8_t other = faceshape_at(ax, ay, az, direction, 0);
    return other >= MB64_FACESHAPE_DOWNUPPERGENTLE_1 &&
           other <= MB64_FACESHAPE_TOPSLAB;
}

static int should_emit_growth_face(const mb64_level_t *level,
                                   const mb64_tile_t *t,
                                   uint8_t direction,
                                   uint8_t faceshape,
                                   uint8_t growth_type) {
    if (growth_type == MB64_GROWTH_NONE || growth_type == MB64_GROWTH_FULL) {
        return 1;
    }
    if (growth_type == MB64_GROWTH_UNCONDITIONAL ||
        growth_type >= MB64_GROWTH_EXTRADECAL_START) {
        return 1;
    }

    const int ax = (int)t->x;
    const int ay = (int)t->y + 1;
    const int az = (int)t->z;
    if (!coords_in_level_range(level, ax, ay, az)) {
        return 1;
    }

    const mb64_tile_t *above = tile_at(ax, ay, az);
    if (above == NULL || above->type == TILE_TYPE_EMPTY) {
        return 1;
    }
    if (mb64_cutout_skip_culling_check(level, t, above, MB64_MESH_FACE_TOP)) {
        return 1;
    }

    if ((faceshape & MB64_FACESHAPE_EMPTY) == 0 &&
        (growth_type == MB64_GROWTH_HALF_SIDE ||
         growth_type == MB64_GROWTH_NORMAL_SIDE) &&
        should_cull_topslab_check(level, t, direction)) {
        return 0;
    }

    uint8_t local_direction = direction;
    if (faceshape > MB64_FACESHAPE_EMPTY) {
        local_direction = (uint8_t)((faceshape - MB64_FACESHAPE_EMPTY) + 1);
    }

    uint8_t other = MB64_FACESHAPE_EMPTY;
    switch (growth_type) {
        case MB64_GROWTH_NORMAL_SIDE:
        case MB64_GROWTH_HALF_SIDE:
            other = faceshape_at(ax, ay, az,
                                 (uint8_t)(rotate_direction(local_direction, t->rot) ^ 1),
                                 0);
            switch (other) {
                ...
                case MB64_FACESHAPE_BOTTOMSLAB:
                    return 0;
                default:
                    return !(growth_type == MB64_GROWTH_HALF_SIDE && faceshape == other);
            }

        case MB64_GROWTH_UNDERSLOPE_CORNER: {
            const uint8_t shape_z = faceshape_at(
                ax, ay, az,
                (uint8_t)(rotate_direction(MB64_MESH_FACE_POS_Z, t->rot) ^ 1),
                0);
            const uint8_t shape_x = faceshape_at(
                ax, ay, az,
                (uint8_t)(rotate_direction(MB64_MESH_FACE_POS_X, t->rot) ^ 1),
                0);
            if (shape_z == MB64_FACESHAPE_TRI_1 && shape_x == MB64_FACESHAPE_TRI_2) {
                return 0;
            }
            return !(faceshape_has_full_bottom(shape_z) ||
                     faceshape_has_full_bottom(shape_x));
        }

        case MB64_GROWTH_DIAGONAL_SIDE:
        case MB64_GROWTH_VSLAB_SIDE:
            other = faceshape_at(ax, ay, az, MB64_MESH_FACE_TOP, 0);
            if (other == MB64_FACESHAPE_FULL) {
                return 0;
            }
            if (other != (growth_type == MB64_GROWTH_DIAGONAL_SIDE
                             ? MB64_FACESHAPE_TOPTRI
                             : MB64_FACESHAPE_TOPHALF)) {
                return 1;
            }
            if (above->type == TILE_TYPE_SCORNER) {
                return 1;
            }
            return (above->rot & 3) != (t->rot & 3);
...
static void emit_shape_face(mb64_mesh_t *mesh, uint32_t *idx,
    ...
    if (!collision_mesh &&
        !should_emit_growth_face(level, t, direction, src->faceshape, src->growth_type)) {
        return;
    }
    mb64_mesh_face_t *face = &mesh->faces[(*idx)++];
    memcpy(face->v, p, sizeof(face->v));
